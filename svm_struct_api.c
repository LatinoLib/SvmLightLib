			      ...
			      KERNEL_PARM *kparm)
{
  ...
  for(i=0;i<sample.n;i++)     /* find highest class label */
    if(sparm->num_classes < (sample.examples[i].y._class+0.1)) 
      sparm->num_classes=sample.examples[i].y._class+0.1;
				    ...
				    STRUCT_LEARN_PARM *sparm)
{
  ...
  LABEL y;
  DOC doc;
  long _class,bestclass=-1,first=1,j;
						     ...
						     STRUCT_LEARN_PARM *sparm)
{
  ...
  LABEL ybar;
  DOC doc;
  long _class,bestclass=-1,first=1;
...
double      loss(LABEL y, LABEL ybar, STRUCT_LEARN_PARM *sparm)
{
  ...
  if(sparm->loss_function == 1) { /* type 1 loss: squared difference */
    return((y._class-ybar._class)*(y._class-ybar._class));
  }
  else {
