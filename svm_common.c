...
MODEL *copy_model(MODEL *model)
{
  ...
  newmodel->n_td_pred = model->n_td_pred;
  newmodel->td_pred = NULL;
  if(model->n_td_pred>0) {
	  newmodel->td_pred = (double *)my_malloc(sizeof(double)*model->n_td_pred);
	  memcpy(newmodel->td_pred, model->td_pred, sizeof(double)*newmodel->n_td_pred);
  }
  return(newmodel);
}
