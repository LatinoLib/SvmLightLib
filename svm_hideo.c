...
double *optimize_qp(QP *, double *, long, double *, LEARN_PARM *, HIDEO_ENV *);
// ---
...
void reset_env(HIDEO_ENV *env)
{
  env->primal=0;
  env->dual=0;
  env->precision_violations=0;
  env->opt_precision=DEF_PRECISION;
  env->maxiter=DEF_MAX_ITERATIONS;
  env->lindep_sensitivity=DEF_LINDEP_SENSITIVITY;
  env->smallroundcount=0;
  env->roundnumber=0;
  env->nonoptimal=0;
  env->buffer=0;
}

HIDEO_ENV *create_env()
{
  HIDEO_ENV *env=my_malloc(sizeof(HIDEO_ENV));
  reset_env(env);
  // ---
  //env->primal = primal;
  //env->dual = dual;
  //env->precision_violations = precision_violations;
  //env->opt_precision = opt_precision;
  //env->maxiter = maxiter;
  //env->lindep_sensitivity = lindep_sensitivity;
  //env->buffer = buffer;
  //env->nonoptimal = nonoptimal;
  //env->smallroundcount = smallroundcount;
  //env->roundnumber = roundnumber;
  // ---
  return env;
}

void free_env(HIDEO_ENV *env)
{
  free(env->primal); 
  free(env->dual);
  free(env->nonoptimal);
...
int optimize_hildreth_despo(long,long,double,double,double,long,long,long,double,double *,
			    ...
			    double *,double *,double *,long *,double *,double *, HIDEO_ENV *);
...
double *optimize_qp(QP *qp,double *epsilon_crit,long nx,double *threshold,LEARN_PARM *learn_parm, HIDEO_ENV *env)
//QP *qp;
//double *epsilon_crit;
...
/* only semi-definite. */
{
  ...
  if(!env->primal) { /* allocate memory at first call */
    env->primal=(double *)my_malloc(sizeof(double)*nx);
    env->dual=(double *)my_malloc(sizeof(double)*((nx+1)*2));
    env->nonoptimal=(long *)my_malloc(sizeof(long)*(nx));
    env->buffer=(double *)my_malloc(sizeof(double)*((nx+1)*2*(nx+1)*2+
  ...
  result=optimize_hildreth_despo(qp->opt_n,qp->opt_m,
				 env->opt_precision,(*epsilon_crit),
				 learn_parm->epsilon_a,env->maxiter,
				 /* (long)PRIMAL_OPTIMAL, */
				 (long)0, (long)0,
				 env->lindep_sensitivity,
				 qp->opt_g,qp->opt_g0,qp->opt_ce,qp->opt_ce0,
				 qp->opt_low,qp->opt_up,env->primal,qp->opt_xinit,
				 env->dual,env->nonoptimal,env->buffer,&progress, env);
  if(verbosity>=3) { 
  ...
  if(result == NAN_SOLUTION) {
    env->lindep_sensitivity*=2;  /* throw out linear dependent examples more */
                            /* generously */
    if(learn_parm->svm_maxqpsize>2) {
      learn_parm->svm_maxqpsize--;  /* decrease size of qp-subproblems */
    }
    env->precision_violations++;
  }

  /* take one round of only two variable to get unstuck */
  if((result != PRIMAL_OPTIMAL) || (!(env->roundnumber % 31)) || (progress <= 0)) {

    env->smallroundcount++;

    result=optimize_hildreth_despo(qp->opt_n,qp->opt_m,
				   env->opt_precision,(*epsilon_crit),
				   learn_parm->epsilon_a,(long)env->maxiter,
  ...
  if(env->precision_violations > 50) {
    env->precision_violations=0;
    (*epsilon_crit)*=10.0; 
    if(verbosity>=1) {
  ...
  if((qp->opt_m>0) && (result != NAN_SOLUTION) && (!isnan(env->dual[1]-env->dual[0])))
    (*threshold)=env->dual[1]-env->dual[0];
  else
    (*threshold)=0;
  ...
  if(verbosity>=4) { /* really verbose */
    ...
    eq=qp->opt_ce0[0];
    for(i=0;i<qp->opt_n;i++) {
      eq+=env->primal[i]*qp->opt_ce[i];
      printf("%f: ",qp->opt_g0[i]);
      for(j=0;j<qp->opt_n;j++) {
	printf("%f ",qp->opt_g[i*qp->opt_n+j]);
      }
      printf(": a=%.30f",env->primal[i]);
      printf(": nonopti=%ld",env->nonoptimal[i]);
      printf(": y=%f\n",qp->opt_ce[i]);
    }
    printf("eq-constraint=%.30f\n",eq);
    printf("b=%f\n",(*threshold));
    printf(" smallroundcount=%ld ",env->smallroundcount);
  }
...
int optimize_hildreth_despo(long n,long m,double precision,double epsilon_crit,double epsilon_a,long maxiter,long goal,
			    ...
			    double *primal,double *init,double *dual,long *lin_dependent,double *buffer,double *progress, HIDEO_ENV *env)
     ...
     //                        before and after */
{
  ...
  if(smallround == SMALLROUND) {
    ...
    if(m>0) { /* for biased hyperplane, pick two variables */
      ...
      lin_dependent[0]=env->smallroundcount % 2;
      lin_dependent[1]=(env->smallroundcount+1) % 2;
    }
  }
  else {
    for(i=0;i<n;i++) {
...
int solve_dual(long n,long m,double precision,double epsilon_crit,long maxiter,double *g,double *g0,double *ce,double *ce0,
		   double *low,double *up,double *primal,double *d,double *d0,double *ig,double *dual,double *dual_old,double *temp,
		   long goal)
