...
void WriteBytes(char *data, int size, WriteByteCallback callback)
{
	for (char *ptr = data; ptr < data + size; ptr++)
	{
		callback(*ptr);
	}
}

void ReadBytes(char *buffer, int size, ReadByteCallback callback)
{
...
SVMLIGHTLIB_API void DeleteFeatureVector(int id)
{
	...
	delete feature_vector; // fixed by Vivek
...
SVMLIGHTLIB_API void SaveModelBinCallback(int model_id, WriteByteCallback callback) // callback support
{
	MODEL *model = GetModel(model_id);
	int ver_len = (int)strlen(VERSION);
	WriteBytes((char *)&ver_len, sizeof(int), callback);
	WriteBytes(VERSION, ver_len, callback);
	WriteBytes((char *)&model->kernel_parm.kernel_type, sizeof(long), callback);
	WriteBytes((char *)&model->kernel_parm.poly_degree, sizeof(long), callback);
	WriteBytes((char *)&model->kernel_parm.rbf_gamma, sizeof(double), callback);
	WriteBytes((char *)&model->kernel_parm.coef_lin, sizeof(double), callback);
	WriteBytes((char *)&model->kernel_parm.coef_const, sizeof(double), callback);
	WriteBytes((char *)&model->kernel_parm.custom, 50, callback);
	WriteBytes((char *)&model->totwords, sizeof(long), callback);
	WriteBytes((char *)&model->totdoc, sizeof(long), callback);
	int sv_num = 1;
	for (int i = 1; i < model->sv_num; i++)
	{
		for (SVECTOR *vec = model->supvec[i]->fvec; vec; vec = vec->next)
	...
	WriteBytes((char *)&sv_num, sizeof(int), callback);
	WriteBytes((char *)&model->b, sizeof(double), callback);
	for (int i = 1; i < model->sv_num; i++)
	{
		for (SVECTOR *vec = model->supvec[i]->fvec; vec; vec = vec->next)
		{
			double tmp = model->alpha[i] * vec->factor;
			WriteBytes((char *)&tmp, sizeof(double), callback);
			int num_feat = 0;
			for (int j = 0; vec->words[j].wnum; j++) { num_feat++; }
...
SVMLIGHTLIB_API int LoadModelBinCallback(ReadByteCallback callback) // callback support
{
	int ver_len;
	ReadBytes((char *)&ver_len, sizeof(int), callback);
	char *version = new char[ver_len + 1];
	ReadBytes(version, ver_len, callback);
	version[ver_len] = 0;
	if (strcmp(version, VERSION))
	{
		perror("Version of model-file does not match version of svm_classify!");
	...
	model->n_td_pred = 0;
	model->td_pred = NULL;
	ReadBytes((char *)&model->kernel_parm.kernel_type, sizeof(long), callback);
	ReadBytes((char *)&model->kernel_parm.poly_degree, sizeof(long), callback);
	ReadBytes((char *)&model->kernel_parm.rbf_gamma, sizeof(double), callback);
	ReadBytes((char *)&model->kernel_parm.coef_lin, sizeof(double), callback);
	ReadBytes((char *)&model->kernel_parm.coef_const, sizeof(double), callback);
	ReadBytes((char *)&model->kernel_parm.custom, 50, callback);
	ReadBytes((char *)&model->totwords, sizeof(long), callback);
	ReadBytes((char *)&model->totdoc, sizeof(long), callback);
	model->sv_num = 0L;
	ReadBytes((char *)&model->sv_num, sizeof(int), callback);
	ReadBytes((char *)&model->b, sizeof(double), callback);
	model->supvec = (DOC **)my_malloc(sizeof(DOC *) * model->sv_num);
	model->alpha = (double *)my_malloc(sizeof(double) * model->sv_num);
	model->index = NULL;
	model->lin_weights = NULL;
	...
	{
		int num_feat;
		ReadBytes((char *)&model->alpha[i], sizeof(double), callback);
		ReadBytes((char *)&num_feat, sizeof(int), callback);
		SvmLight::WORD *words = (SvmLight::WORD *)my_malloc(sizeof(SvmLight::WORD) * (num_feat + 1));
		for (int j = 0; j < num_feat; j++)
		{
			ReadBytes((char *)&words[j].wnum, sizeof(FNUM), callback);
			ReadBytes((char *)&words[j].weight, sizeof(FVAL), callback); 
		}
		words[num_feat].wnum = 0;
		int comment_len;		
		ReadBytes((char *)&comment_len, sizeof(int), callback);
		char *comment = new char[comment_len + 1];
...
SVMLIGHTLIB_API void SaveMulticlassModelBinCallback(int model_id, WriteByteCallback callback) // callback support
{
	StructModelWithParams *model_with_params = GetStructModelWithParams(model_id);
	STRUCTMODEL *model = model_with_params->first;
	MODEL *svm_model = model->svm_model;
	...
	int ver_len = (int)strlen(INST_VERSION);
	WriteBytes((char *)&ver_len, sizeof(int), callback);
	WriteBytes(INST_VERSION, ver_len, callback);
	WriteBytes((char *)&params->num_classes, sizeof(int), callback);
	WriteBytes((char *)&params->num_features, sizeof(int), callback);
	WriteBytes((char *)&params->loss_function, sizeof(int), callback);
	WriteBytes((char *)&svm_model->kernel_parm.kernel_type, sizeof(long), callback);
	WriteBytes((char *)&svm_model->kernel_parm.poly_degree, sizeof(long), callback);
	WriteBytes((char *)&svm_model->kernel_parm.rbf_gamma, sizeof(double), callback);
	WriteBytes((char *)&svm_model->kernel_parm.coef_lin, sizeof(double), callback);
	WriteBytes((char *)&svm_model->kernel_parm.coef_const, sizeof(double), callback);
	WriteBytes((char *)&svm_model->kernel_parm.custom, 50, callback);
	WriteBytes((char *)&svm_model->totwords, sizeof(long), callback);
	WriteBytes((char *)&svm_model->totdoc, sizeof(long), callback);
	int sv_num = 1;
	SVECTOR *vec;
	for (int i = 1; i < svm_model->sv_num; i++)
	{
	...
	WriteBytes((char *)&sv_num, sizeof(int), callback);
	WriteBytes((char *)&svm_model->b, sizeof(double), callback);
	for (int i = 1; i < svm_model->sv_num; i++)
	{
		for (vec = svm_model->supvec[i]->fvec; vec; vec = vec->next)
		{
...
SVMLIGHTLIB_API int LoadMulticlassModelBinCallback(ReadByteCallback callback) // callback support
{
	STRUCTMODEL *model = new STRUCTMODEL();
	STRUCT_LEARN_PARM *params = new STRUCT_LEARN_PARM();
	int ver_len;
	...
	svm_model->n_td_pred = 0;
	svm_model->td_pred = NULL;
	ReadBytes((char *)&params->num_classes, sizeof(int), callback);
	ReadBytes((char *)&params->num_features, sizeof(int), callback);
	ReadBytes((char *)&params->loss_function, sizeof(int), callback);
	ReadBytes((char *)&svm_model->kernel_parm.kernel_type, sizeof(long), callback);
	ReadBytes((char *)&svm_model->kernel_parm.poly_degree, sizeof(long), callback);
	ReadBytes((char *)&svm_model->kernel_parm.rbf_gamma, sizeof(double), callback);
	ReadBytes((char *)&svm_model->kernel_parm.coef_lin, sizeof(double), callback);
	ReadBytes((char *)&svm_model->kernel_parm.coef_const, sizeof(double), callback);
	ReadBytes((char *)&svm_model->kernel_parm.custom, 50, callback);
	ReadBytes((char *)&svm_model->totwords, sizeof(long), callback);
	ReadBytes((char *)&svm_model->totdoc, sizeof(long), callback);
	svm_model->sv_num = 0L;
	ReadBytes((char *)&svm_model->sv_num, sizeof(int), callback);
	ReadBytes((char *)&svm_model->b, sizeof(double), callback);
	svm_model->supvec = (DOC **)my_malloc(sizeof(DOC *) * svm_model->sv_num);
	svm_model->alpha = (double *)my_malloc(sizeof(double) * svm_model->sv_num);
	svm_model->index = NULL;
	svm_model->lin_weights = NULL;	
